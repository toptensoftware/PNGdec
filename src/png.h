...
#define PNG_STATIC
    ...
    int32_t seekMem(PNGFILE* pFile, int32_t iPosition);
    int32_t readFLASH(PNGFILE* pFile, uint8_t* pBuf, int32_t iLen);
    int32_t readRAM(PNGFILE* pFile, uint8_t* pBuf, int32_t iLen);

    int PNGInit(PNGIMAGE* pPNG);
    int PNG_decode(PNGIMAGE* pImage, void* pUser, int iOptions);
    uint8_t PNGMakeMask(PNGDRAW* pDraw, uint8_t* pMask, uint8_t ucThreshold);
